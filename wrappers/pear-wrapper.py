...
out_base, out_end = assembled.rsplit('.f', 1)
out_end = '.f' + out_end
...
unassembled_r1    = snakemake.output.get('unassembled_r1', out_base + '.unassembled_r1' + out_end)
unassembled_r2    = snakemake.output.get('unassembled_r2', out_base + '.unassembled_r2' + out_end)
...
# resources are req'd to be integers. change mem format for pear:
max_mem = str(snakemake.resources.get('mem_mb', "4000")) # 4GB
if "000" in max_mem:
    max_mem = max_mem.rsplit("000", 1)[0] + "G"
else:
    max_mem = max_mem + "M"
